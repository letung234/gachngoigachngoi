...
export const handleImageProduct = (product) => {
  // Cloudinary returns full URLs, no need to prepend HOST
  return product
}

const removeImageProduct = async (image: string) => {
...
const removeManyImageProduct = async (images: string[]) => {
  if (images !== undefined && images.length > 0) {
    try {
      await deleteManyFromCloudinary(images)
    } catch (err) {
...
const uploadProductImage = async (req: Request, res: Response) => {
  const url = await uploadFileCloudinary(req, FOLDERS.PRODUCT)
  const response = {
...
const uploadManyProductImages = async (req: Request, res: Response) => {
  const urls = await uploadManyFileCloudinary(req, FOLDERS.PRODUCT)
  const response = {
