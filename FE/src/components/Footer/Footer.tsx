...
export default function Footer() {
  ...
  const brandName = config?.footer?.brandName || config?.siteName || 'Gạch Ngói Việt'
  const brandSlogan = config?.footer?.brandSlogan || config?.siteSlogan || 'Tinh hoa làng nghề'
  const aboutText = config?.footer?.about || 'Kế thừa và phát huy tinh hoa nghề làm gạch ngói truyền thống Việt Nam. Sản phẩm thủ công, bền vững theo thời gian.'
  const copyright = config?.footer?.copyright || `\u00A9 ${new Date().getFullYear()} Gạch Ngói Việt. Tất cả quyền được bảo lưu.`
  const isShowSocialIcons = config?.footer?.showSocialIcons !== false

  const productLinks = config?.footer?.productLinks?.length
    ? config.footer.productLinks
    : DEFAULT_PRODUCT_LINKS

  const companyLinks = config?.footer?.companyLinks?.length
    ? config.footer.companyLinks
    : DEFAULT_COMPANY_LINKS

  const contactAddress = config?.contact?.address || 'Làng nghề Bát Tràng, Gia Lâm, Hà Nội'
  const contactPhone = config?.contact?.phone || '0987 654 321'
  const contactEmail = config?.contact?.email || 'info@gachngoiviet.vn'

  const socialFacebook = config?.social?.facebook || '#'
  const socialZalo = config?.social?.zalo || '#'
  const socialYoutube = config?.social?.youtube || '#'
        ...
        <div className='grid grid-cols-1 gap-8 sm:gap-10 md:gap-12 md:grid-cols-2 lg:grid-cols-4'>
          ...
          <div className='sm:col-span-1 md:col-span-2 lg:col-span-1'>
            <Link to='/' className='mb-6 inline-flex items-center gap-3'>
              {config?.logo ? (
                <img src={config.logo} alt={brandName} className='h-12 w-auto object-contain' />
              ) : (
              ...
              <div>
                <h2 className='font-serif text-lg md:text-xl font-bold'>{brandName}</h2>
                <p className='text-xs text-cream-light/70'>{brandSlogan}</p>
              </div>
            </Link>
          ...
          <div className='sm:col-span-1 md:col-span-2 lg:col-span-1'>
            ...
            <ul className='space-y-3'>
              <li className='flex items-start gap-3'>
                ...
                <span className='text-sm text-cream-light/70'>{contactAddress}</span>
              </li>
              <li className='flex items-center gap-3'>
                ...
                <a href={`tel:${contactPhone.replace(/\s/g, '')}`} className='text-sm text-cream-light/70 hover:text-cream-light'>
                  {contactPhone}
                </a>
              </li>
              <li className='flex items-center gap-3'>
                ...
                <a href={`mailto:${contactEmail}`} className='text-sm text-cream-light/70 hover:text-cream-light'>
                  {contactEmail}
                </a>
              </li>
            </ul>
